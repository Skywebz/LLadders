...
public class BlockLadderDispenser extends BlockContainer {
	...
	public boolean onBlockActivated(World par1World, int par2, int par3, int par4, EntityPlayer par5EntityPlayer, int par6, float par7, float par8, float par9) {
		...
		TileEntityLadderDispenser te = (TileEntityLadderDispenser)par1World.getBlockTileEntity(par2, par3, par4);
	...
	private boolean canRemoveLadder(World world, int x, int y, int z, int meta) {
		...
		return world.getBlockId(x, y, z) == LLadders.blockRopeLadder.blockID;
	}
	
	private boolean canSetLadder(World world, int x, int y, int z, int meta) {
		
		if (world.getBlockId(x, y, z) == LLadders.blockRopeLadder.blockID) {	// We want to check if there is ladders below as well.
	...
	private boolean insertLadderToDispenser(TileEntityLadderDispenser te, ItemStack itemstack) {
		...
		for (int i = 0; i < te.getSizeInventory(); i++) {
			...
			if (te.getStackInSlot(i) == null) {
				...
				ItemStack stack = new ItemStack(itemstack.itemID, te.getStackInSlot(i).stackSize + 1, itemstack.getItemDamage());
	...
	private ItemStack extractLadderFromDispenser(TileEntityLadderDispenser te) {
		...
		for (int i = te.getSizeInventory() - 1; i >= 0; i--) {
			...
			if ((te.getStackInSlot(i) != null) && (te.getStackInSlot(i).itemID == LLadders.blockRopeLadder.blockID)) {
