	...
	public void onPlayerLogin(PlayerEvent.PlayerLoggedInEvent event) {
		...
		if (!_notificationDisplayed && _updateChecker.checkComplete()) {
			...
			if (_updateChecker.isNewVersionAvailable()) {
				...
				ChatComponentText msg = new ChatComponentText("[" + References.MOD_NAME + "] New version is available: " + _updateChecker.newVersion().toString());
				event.player.addChatMessage(msg);
				msg = new ChatComponentText("[" + References.MOD_NAME + "] " + _updateChecker.newVersion().toString() + ": " +_updateChecker.newVersion().description());
				event.player.addChatMessage(msg);
			}
		}
	}
