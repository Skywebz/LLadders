...
public class ModVersion implements Comparable<ModVersion> {
	...
	public int compareTo(ModVersion par1) {
		...
		if (this.major() != par1.major())
			return this.major() < par1.major() ? -1 : 1;
		if (this.minor() != par1.minor())
			return this.minor() < par1.minor() ? -1 : 1;
		if (this.patch() != par1.patch())
			return this.patch() < par1.patch() ? -1 : 1;
		return 0;
	}
